...
export class AuthService {
  ...
  login(email: string, password: string, rememberMe: boolean): Observable<LoginResponseType | DefaultResponseType> {
    return this.http.post<LoginResponseType | DefaultResponseType>(config.api + 'login', {
      email,
      password,
      rememberMe
    })
  ...
  signup( name: string, email: string, password: string): Observable<LoginResponseType | DefaultResponseType> {
    return this.http.post<LoginResponseType | DefaultResponseType>(config.api + 'signup', {
      name,
      email,
      password,
    })
  ...
  logout(): Observable<DefaultResponseType> {
    ...
    if (token && token.refreshToken) {
      return this.http.post<DefaultResponseType>(config.api + 'logout', {
  ...
  refresh(): Observable<DefaultResponseType | LoginResponseType> {
    ...
    if (tokens && tokens.refreshToken) {
      return this.http.post<DefaultResponseType | LoginResponseType>(config.api + 'refresh', {
